...
<!-- TODO Courtney - Add adding an action from side lists like in Events. Use similar descriptions *I think what you're asking for is the above 'Adding an Existing Action' section which is very similar to the 'Adding an Existing Event' section. If you'd like something else, please let me know. -Courtney 12/3/2020-->
...
<!-- TODO Courtney - Pictures in the following section are missing Completed- Courtney- 12/2/2020-->
## Removing an Action
**Option 1:** <br>
...
is marked as "Exit" when it occurs, (See [State Properties](./states.md#state-properties) and [Editing an Event](./events.md#editing-an-event) for more information<!-- TODO Courtney, add reference to applicable event and state editing section *Completed -Courtney -12/3/2020-->) then the model is
...
the "Immediate Actions" list (See [Immediate and Event Actions](#immediate-and-event-actions) for more information<!-- TODO Courtney, add reference immediate actions section *Added an Immediate and Event Actions section to the top of this .md and referenced it. Completed -Courtney -12/3/2020 -->) will always be adding to the current states because there is no event with an exit property and it would be bad modeling to exit a state immediately after entering, making it possible for other immediate actions to not be executed.
...
- **State Table:** <!-- TODO Courtney - All this info should go under the types, transition section. Completed- Courtney- 12/2/2020 -->
...
<h3>External Simulation Message <img src="/images/Modeling/actions/extSim.PNG" alt="Change Variable Value" width="25"/></h3>  
...
![External Simulation Message Options](/images/Modeling/actions/ExtSimMessageOptions.PNG)
...
![Run Application Options](/images/Modeling/actions/RunApplicationOptions.PNG)
...
It is recommened that you use a c# compiler to test and debug code before entering it. See the video "RunNotepad" and model "RunExe_Notepad.json" for detailed instructions
and an example.
...
This list will not include pre-existing variables such as CurTime. See [Pre-existing Variables](./variables.md#pre-existing-variables) for a full list 
of pre-existing variables. It will include the all of the variables that you create and are listed in the "All" Tab of the Left Navigation Frame. 
...
<!--TODO Courney - I updated the website so that the correct icons show up for the actions in both the immediate action and event actions
update the icon list and put them in the headers for each of the types. 
Icons are in EMRALD_Site/images
 Transition -TransitionArrow.png
 Change variable value - varCond.png
 3D Sim Message - ExtSim.png
 Run Application - cogwheel.png
