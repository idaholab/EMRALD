...
::: tip NOTE
Before opening a file in the web-based model editor to fix an error, close the solver. Not doing so can cause the file to be locked and not update since it may be simultaneously locked by the solver.
:::

Errors displayed are only errors that make the model technically incorrect, such as fields being left empty or a syntax error within coded sections. They are all structured to say that the solver, "Failed to load model", then it identifies what Event, Action, or other piece is incorrect, and finally, it states what the error was. Some examples of error messages and how to fix them are tabulated below.
|Error|Fix|
|---|---|
...
- **Runs:** The total number of runs, Start state to End state, that you would like to simulate. The default value is 1000.
- **Max Sim Time:** The maximum total simulation time. This is not computing time. The format for entering simulation time is [days.hh:mm:ss.ms]. Do not put 00.24:00:00 for one day; instead, put 1.00:00:00. The default value is 365.00:00:00.
...
- **Path Results Loc:** The directory location of the basic results. The default location and title of the text file is c:\temp\NewSimResults.txt. This can be changed directly in the field where the default path is shown or you can click "Open" to use a File Explorer to navigate to the location you would like to save your file. See [Path Results](#path-results) for more information on the actual results file.
- **Seed:** You can prescribe a seed for consistent results or leave it blank for a random seed to be generated.
- **Debug:** Check the box to have a file saved for debugging your model. You can select either the radio button for a Basic debug file or a Detailed debug file that will record either only the movement between states or also include the triggered actions and events. You can also define which runs you would like to have the file output for. Indexing starts at 1, and the default entry is "From Run: 1 To Run: [Number of Runs]". See [Debug File](#debug-file) for more information on the file that is generated from selecting this option.
...
### Path Results
The path results will display the paths taken to arrive at each key state reached. This will include nested entries for each movement between states for each cause path and each key state, as shown below.<br/>
...
- **"keyStates":** Each key state will have a header entry that is structured like the following:
    ...
    - **"watchVariables":** A list of all variables and their values at the end of each run when the key state was reached. It is structured like this:<br/>
...
- **"causes:"** After the header is the "causes" section, which is a cascading list or lists of state transitions that occured in reverse order to lead to the key state. Each list entry will be structured as shown below.<br/>
    ...
    **"fromState:** {<br/>
        ...
        **"times":** This lists, for each run, the state was reached and the amount of simulation time it took to reach the state via the path in the description.<br/>
        **"timeMean":** The mean time of all the times listed.<br/>
        **"timeStdDeviation":** The standard deviation of all the times listed.<br/>
        **"watchVariables":** A list of all variables and their values when the state was reached. It is structured in the same way as the "watchVariables" section for the **Key State** header.
