...
Password - Default is "secret". This can be assigned in EMRALD when starting up through command line. To see command line options, run the exe with the command -help.


## Message Format
The messages are in JSON format and there are two types of messages: (1) Action Messages, messages from EMRALD for the client to do something with, (2) Event Messages, messages from the client that EMRALD needs to respond to. The message format can be found in c# format under the MessageDeffLib project in MsgWrapper.cs, and a schema to test against is found in MessageProtocol.json. The following sections go over the different varibles and types for the messages.
...
- **atTimer** - Informs the coupled simulation of the next time that a change needs to occur in EMRALD. This is a callback timer that the coupled application must respond with an atTimer message and wait for a response when its simulation reaches this global time. 
...
- **atPauseSim** - Pauses the current simulation (for future multiple coupled simulations, not currently in use).

- **atContinue** - Tells the application to continue the simulation when EMRALD is done on its side.
...
- **atTerminate** - When EMRALD is done running its simulations and is shutting down, the coupled application should do the same.
...
- **etEndSim** - Tells EMRALD the simulation is complete either because of internal conditions, or the max time specified by the user was reached.

- **etPing** - Is a repose to a ping from EMRALD.

- **etStatus** - This message can be sent at any time by the coupled simulation. It must be sent in response to an atStatus message. A message type of stError will cause EMRALD to end its simulation.
...
**SimTime** - Measure the chronology of the client simulation time, starting from zero seconds when it begins at the first atContinue message from EMRALD. The "time" value in the messages is the EMRALD global time from the beginning of the EMRALD simulation.
...
**CallbackItem** - Save the itemData (name, value). This is given when an atTimer message is sent. Must provide the name and value when sending a etTimer message at the callback time.

**EndTime** - The maximum local time, to guard against runaway simulations, at which AVERT must terminate its simulation. This is specified by the user and passed during the opening.
...
### stIdle
Initial startup condition or after running a simulation. 
...
By selecting and filling out the top options, a message can be automatically constructed. The options depend on the type of message selected by the user. Clicking the "Generate Message" button autogenerates the JSON message in the box below. 
