...
On the Left Navigation Frame, expand the External Sims section. Next, follow either Option 1 or Option 2.

**Option 1:** Double-click on the External Simulation that you want to edit.

**Option 2:** Right-click on the External Simulation that you want to edit. A drop-down menu will appear and click "Edit Properties...".<br>
...
First, make sure that the External Simulation Link is created by following the steps in [Creating a New External Simulation](#creating-a-new-external-simulation). 
...
"Immediate actions" label or on the event you would like to add it to. Click "New Action". <br>
...
The fields in the bottom half of the Action Properties should have changed to look like the following. Each type of message requires the "External Sim" that the message
will be sent to. 
...
The various actions available in the "Sim Action" drop-down menu will explained later. The focus now will be on "Open Sim", which allows sends a message to start 
...
### Ping
This message is for testing and could be used to tell the connected simulation to output a status.
...
whenever the local time reaches the next specified callback time. Whenever it receives new callback times, the old ones must be removed. 
