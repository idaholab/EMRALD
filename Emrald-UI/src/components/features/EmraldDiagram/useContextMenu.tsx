...
const useContextMenu = (getStateNodes?: () => void) => { // Get state nodes function is needed if deleting or removing a state
  ...
  **/
  ...
  const onActionsHeaderContextMenu = (e: React.MouseEvent, type: "event" | "immediate") => {
    e.preventDefault();
    e.stopPropagation();
    setMenu({
      mouseX: e.clientX,
    ...
    const defaultOptions = [
      { label: 'New Event', action: () => {addWindow("New Event", <EventForm />); closeContextMenu(); }},
      { label: 'New Action', action: () => {addWindow("New Action", <ActionForm />); closeContextMenu(); }},
    ];
  ...
  const onEventContextMenu = (e: React.MouseEvent, state: State, event?: Event) => {
    ...
    const defaultOptions = [
      { label: 'Event Properties', action: () => {addWindow(event.name, <EventForm eventData={event}/>); closeContextMenu(); }},
      { label: 'New Action', action: () => {addWindow("New Action", <ActionForm />); closeContextMenu(); }, isDivider: true },
      { label: 'Move Up', action: () => {moveEvent(state, event.name, 'up'); closeContextMenu();}},
      { label: 'Move Down', action: () => {moveEvent(state, event.name, 'down'); closeContextMenu();}, isDivider: true },
      { label: 'Remove Event', action: () => { removeEventItem(event, state); closeContextMenu(); } },
      { label: 'Delete Event', action: () => { showDeleteConfirmation(event, state); closeContextMenu(); } },
    ];
  ...
  const onActionContextMenu = (event: React.MouseEvent, state: State, action: Action, type: 'immediate' | 'event') => {
    event.preventDefault(); // Prevent native context menu from showing
    ...
    const defaultOptions = [
      { label: 'Action Properties',  action: () => {addWindow(action.name, <ActionForm actionData={action}/>); closeContextMenu(); }, isDivider: true },
      ...
      { label: 'Remove Action', action: () => { removeActionItem(action, type, state); closeContextMenu(); } },
      { label: 'Delete Action', action: () => { showDeleteConfirmation(action, state, type); closeContextMenu(); } },
    ];
  ...
  **/
  ...
  const removeStateItem = (stateToRemove: State) => {
    ...
    if (stateToRemove.id && getStateNodes) {
      currentDiagram.value.states = currentDiagram.value.states.filter(state => state !== stateToRemove.name);
      getStateNodes();
  ...
  const removeActionItem = (actionToRemove: Action, actionType: 'immediate' | 'event', state: State) => {
    if (!actionToRemove) { return; }
    if (actionToRemove && actionType) {
      if (actionType === 'event') {
        state.eventActions.forEach((eventAction) => {
          if (eventAction.actions.includes(actionToRemove.name)) {
            eventAction.actions = eventAction.actions.filter(action => action !== actionToRemove.name);
          }
        });
      }
      if (actionType === 'immediate') {
        state.immediateActions = state.immediateActions.filter(action => action !== actionToRemove.name);
      }
      updateState(state);
    }
  }
  ...
  const deleteItem = () => {
    ...
    if (itemToDelete.id && isState(itemToDelete) && getStateNodes) {
      currentDiagram.value.states = currentDiagram.value.states.filter(state => state !== itemToDelete.name);
      getStateNodes();
      updateDiagramDetails(currentDiagram.value);
      ...
      if (actionTypeToModify === 'event') {
        stateToModify.eventActions.forEach((eventAction) => {
          if (eventAction.actions.includes(itemToDelete.name)) {
            eventAction.actions = eventAction.actions.filter(action => action !== itemToDelete.name);
          }
        });
      }
      if (actionTypeToModify === 'immediate') {
        stateToModify.immediateActions = stateToModify.immediateActions.filter(action => action !== itemToDelete.name);
      }
      updateState(stateToModify);
  ...
  */  
  const showDeleteConfirmation = (itemToDelete: Event | State | Action, state?: State, actionType?: 'immediate' | 'event') => {
