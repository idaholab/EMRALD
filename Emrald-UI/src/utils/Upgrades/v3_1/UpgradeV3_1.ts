...
export function UpgradeV3_1(modelTxt: string) {
  return {
    newModel: JSON.stringify(
      UpgradeV3_1_Recursive(JSON.parse(modelTxt) as EMRALD_ModelV3_0),
    ),
    errors: [],
  };
}

function UpgradeV3_1_Recursive(oldModel: EMRALD_ModelV3_0) {
  // Note: The reassignment of the objType properties here is just to make TypeScript happy
  const upgradeModel = (oldModel: EMRALD_ModelV3_0) => ({
    ...oldModel,
