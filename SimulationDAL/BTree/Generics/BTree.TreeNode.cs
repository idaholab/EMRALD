...
namespace Sop.Collections.Generic.BTree
{
    ...
    internal partial class BTreeAlgorithm<TKey, TValue>
    {
        ...
        internal class TreeNode
        {
            ...
            internal protected TreeNode(BTreeAlgorithm<TKey, TValue> ParentTree, TreeNode ParentNode)
            {
            ...
            internal protected void Initialize(BTreeAlgorithm<TKey, TValue> Btree, TreeNode ParentObj)
            {
                if (Slots == null)
            ...
            internal protected void Add(BTreeAlgorithm<TKey, TValue> ParentBTree,
                ...
                while (true)
                {
                    ...
                    if (NoOfOccupiedSlots > 1)
                    {
                        if (ParentBTree.SlotsComparer != null)
                            Index = Array.BinarySearch(CurrentNode.Slots, 0, NoOfOccupiedSlots, Item, ParentBTree.SlotsComparer);
                        else
                    ...
                    else if (NoOfOccupiedSlots == 1)
                    {
                        ...
                        else
                        {
                            try
                            {
                                if (System.Collections.Generic.Comparer<TKey>.Default.Compare(CurrentNode.Slots[0].Key, Item.Key) < 0)
                                    Index = 1;
                            }
                            catch (Exception)
                            {
                    ...
                    if (Parent != null)
                    {
                        ...
                        if (iIsThereVacantSlot > 0)
                        {
                            ...
                            if (iIsThereVacantSlot == 1)
                                ...
                                DistributeToLeft(ParentBTree, ParentBTree.TempSlots[ParentBTree.SlotLength]);
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                // assign the new children nodes.
                                ParentBTree.TempParentChildren[(int)Sop.Collections.BTree.ChildNodes.LeftChild] = this;
                                ParentBTree.TempParentChildren[(int)Sop.Collections.BTree.ChildNodes.RightChild] = RightNode;
            ...
            internal protected bool Search(BTreeAlgorithm<TKey, TValue> ParentBTree,
            ...
            internal protected bool MoveFirst(BTreeAlgorithm<TKey, TValue> ParentBTree)
            {
                TreeNode Node = this;
            ...
            internal protected bool MoveNext(BTreeAlgorithm<TKey, TValue> ParentBTree)
            {
                TreeNode CurrentNode = this;
            ...
            /// <param name="ParentBTree">Parent BTree</param>
            /// <returns>true if successful, else false</returns>
            internal protected bool MovePrevious(BTreeAlgorithm<TKey, TValue> ParentBTree)
            {
            ...
            /// <param name="ParentBTree">Parent BTree</param>
            /// <returns>true if successful, else false</returns>
            internal protected bool MoveLast(BTreeAlgorithm<TKey, TValue> ParentBTree)
            {
                TreeNode Node = this;
                while (Node.Children != null)
                ...
                if (Parent != null)
                {
                    ...
                    if (IndexOfNode == -1 || Parent.Children[IndexOfNode] != this)
                    {
                        for (IndexOfNode = 0; IndexOfNode <= SlotLength &&
                            ...
                            if (Parent.Children[IndexOfNode] == this) break;
                    }
                    if (IndexOfNode >= 0 || IndexOfNode > Parent.count + 1)
            ...
            //public class CompareNode : System.Collections.Generic.IComparer<TreeNode>
            //{
            //    public CompareNode(BTreeAlgorithm<TKey, TValue> Parent)
            //    {
            //        this.Parent = Parent;
            //    }
            //    public int Compare(TreeNode x, TreeNode y)
            //    {
            //        int xID = x.GetHashCode(), yID = y.GetHashCode();
            //        if (xID == yID)
            //            return 0;
            //        else if (xID < yID)
            //            return -1;
            //        return 1;
            //    }
            //    BTreeAlgorithm<TKey, TValue> Parent;
            //}
            ...
            internal void Promote(BTreeAlgorithm<TKey, TValue> ParentBTree, byte Position)
            {
                ...
                if (NoOfOccupiedSlots < (byte)ParentBTree.SlotLength)
                {
                    ...
                    Children[Index + 1] = ParentBTree.TempParentChildren[(int)Sop.Collections.BTree.ChildNodes.RightChild];
                    count++;
                    return;	// successful
                }
                else
            ...
            private void DistributeToLeft(BTreeAlgorithm<TKey, TValue> ParentBTree, BTreeItem<TKey, TValue> Item)
            {
                byte SlotLength = ParentBTree.SlotLength;
            ...
            //private void DistributeToLeft(BTreeAlgorithm<TKey, TValue> ParentBTree, KeyValuePair<TKey, TValue> Item)
            //{
            //    TreeNode CurrentNode = this;
            //    byte SlotLength = ParentBTree.SlotLength;
            //    KeyValuePair<TKey, TValue> CurrentItem = Item;
            //    while (true)
            //    {
            //        if (CurrentNode.IsFull(SlotLength))
            //        {	// counter-clockwise rotation..
            //            //	----
            //            //	|  |
            //            //	-> |
            //            // NOTE: we don't check for null returns as this method is called only when there is vacant in left
            //            byte IndexOfNode = (byte)(CurrentNode.GetIndexOfNode(ParentBTree) - 1);
            //            CurrentItem = CurrentNode.Parent.Slots[IndexOfNode];
            //            CurrentNode = CurrentNode.GetLeftSibling(ParentBTree);
            //            //GetLeftSibling(ParentBTree).DistributeToLeft(ParentBTree, Parent.Slots[GetIndexOfNode(ParentBTree) - 1]);

            //            CurrentNode.Parent.Slots[IndexOfNode] = Slots[0];
            //            MoveArrayElements(Slots, 1, 0, (ushort)(SlotLength - 1));
            //            Slots[count - 1] = null;
            //        }
            //        else
            //            CurrentNode.count++;
            //        CurrentNode.Slots[CurrentNode.count - 1] = Item;
            //    }
            //}
            ...
            private void DistributeToRight(BTreeAlgorithm<TKey, TValue> ParentBTree, BTreeItem<TKey, TValue> Item)
            {
                byte SlotLength = ParentBTree.SlotLength;
                if (IsFull(SlotLength))
                {	// clockwise rotation..
                    ...
                    //	|  |
                    //	| <-
                    GetRightSibling(ParentBTree).DistributeToRight(ParentBTree, Parent.Slots[GetIndexOfNode(ParentBTree)]);
                    Parent.Slots[GetIndexOfNode(ParentBTree)] = Slots[count - 1];
                }
                else
                    count++;
            ...
            /// </summary>
            /// <param name="ParentBTree">Parent BTree</param>
            private void PullFromLeft(BTreeAlgorithm<TKey, TValue> ParentBTree)
            {
                byte i = count;
                ...
                else
                {
                    ...
                    {	// the following code should process unbalanced sibling branch.
                        ...
                        i = (byte)(GetIndexOfNode(ParentBTree) - 1);
            ...
            private void PullFromRight(BTreeAlgorithm<TKey, TValue> ParentBTree)
            {
                byte i = count;
                if (i > 1)
                {
        ...
        internal class TreeRootNode : TreeNode
        {
            ...
            internal protected TreeRootNode(BTreeAlgorithm<TKey, TValue> ParentTree) : base(ParentTree) { }
