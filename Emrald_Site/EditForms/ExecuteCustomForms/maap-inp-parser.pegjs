...
SingleLineComment = ("//" / "**" / "!" / "C") v:FreeCharacter*
...
Reserved = END
NotReserved = v:(!Reserved [a-zA-Z]+) {
	return v[1].join('');
}
ParameterName = first:NotReserved rest:(_ NotReserved)? {
	let name = first;
	if (rest) {
    	name
    }
...
Statement = SensitivityStatement
    ...
    / InitiatorsStatement
SensitivityStatement = SENSITIVITY __ v:(ON / OFF) {
	return {
    	type: "sensitivity",
        value: v
    }
}
TitleStatement = TITLE __ v:TitleBlock __ END {
	return {
    	type: "title",
        value: v,
    }
}
TitleBlock = v:FreeCharacter* {
	return extractList(v, 1).join('');
}
...
ParameterChangeStatement = PARAMETER_CHANGE __ v:ParameterChangeBlock __ END {
	return {
    	type: "parameter_change",
        value: v,
    }
}
ParameterChangeBlock = ""
TimeStatement = t:(START_TIME / END_TIME) _ IS _ v:NumericLiteral _ "." {
	return {
    	type: "time",
        time: t,
        value: v,
...
PrintIntervalStatement = PRINT_INTERVAL _ IS _ v:NumericLiteral _ "." {
	return {
    	type: "print_interval",
        value: v,
    }
}
InitiatorsStatement = INITIATORS __ body:InitiatorsBody? __ END {
	return {
    	type: "initiators",
        value: body,
    }
}
InitiatorsBody = first:ParameterName rest:(__ InitiatorsBody)? {
	return first
}
