...
function OnLoad(dataObj) {
  ...
  scope.$apply(function () {
    ...
    scope.name = diagramData.name;
		...
		scope.diagramTypes = diagramData.diagramLabels;
    ...
    if (scope.diagramTypes && scope.diagramTypes.length > 0) {
      var dInst = scope.diagramTypes.find(function (dt) {
					if (dataObj.diagramLabel) {
							return (dt.value == dataObj.diagramType && dt.name == dataObj.diagramLabel);
					}
					else {
							return (dt.value == dataObj.diagramType);
					}
      });
