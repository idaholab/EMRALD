...
const coverageMode = process.argv.indexOf('--coverage') >= 0;
...
http
    ...
    if (request.url === '/coverage') {
      ...
      switch (extname) {
        case '.js':
          ...
          // Redirect to instrumented code if generating coverage
          if (coverageMode && filePath.indexOf('tests') < 0) {
  ...
  .listen(8080, () => {
    ...
    if (coverageMode) {
      console.log('Running in coverage mode');
    }
  });
