...
(function (Navigation) {
  var Sidebar = (function (_super) {
      ...
      for (var i = 0; i < diagramList.length; i++) {
        ...
        diagramList[i].ActionList.forEach((a) => {
          if (a.Action.required && !this.ActionExists(a.Action.name)) {
            diagramList[i].disabledReasons.push(
              `Action '${a.Action.name}' must already exist.`,
            );
          }
        });
        diagramList[i].DiagramList.forEach((d) => {
          if (d.Diagram.required && !this.DiagramExists(d.Diagram.name)) {
            diagramList[i].disabledReasons.push(
              `Diagram '${d.Diagram.name}' must already exist.`,
            );
          }
        });
        diagramList[i].EventList.forEach((e) => {
          if (e.Event.required && !this.EventExists(e.Event.name)) {
            diagramList[i].disabledReasons.push(
              `Event '${e.Event.name}' must already exist.`,
            );
          }
        });
        diagramList[i].ExtSimList.forEach((es) => {
          if (es.ExtSim.required && !this.ExtSimExists(es.ExtSim.name)) {
            diagramList[i].disabledReasons.push(
              `ExtSim '${es.ExtSim.name}' must already exist.`,
            );
          }
        });
        diagramList[i].LogicNodeList.forEach((ln) => {
          if (
            ln.LogicNode.required &&
            !this.LogicNodeExists(ln.LogicNode.name)
          ) {
            diagramList[i].disabledReasons.push(
              `Logic Node '${ln.LogicNode.name}' must already exist.`,
            );
          }
        });
        diagramList[i].StateList.forEach((s) => {
          if (s.State.required && !this.StateExists(s.State.name)) {
            diagramList[i].disabledReasons.push(
              `State '${s.State.name}' must already exist.`,
            );
          }
        });
        diagramList[i].VariableList.forEach((s) => {
          if (s.Variable.required && !this.VariableExists(s.Variable.name)) {
            diagramList[i].disabledReasons.push(
              `Variable '${s.Variable.name}' must already exist.`,
            );
          }
        });
      ...
      var wnd = mxWindow.createFrameWindow(
        ...
        function (btn, outDataObj) {
          if (btn === 'OK') {
            ...
            outDataObj.states = [];
            if (
              outDataObj.diagramType == 'dtComponent' ||
              outDataObj.diagramType == 'dtSystem'
            )
              outDataObj.singleStates = [];
            if (!outDataObj.diagramLabel)
              outDataObj.diagramLabel = outDataObj.diagramType.substring(
                2,
                outDataObj.diagramType.length,
              );
            ...
            if (
              outDataObj.diagramTemplate &&
              outDataObj.diagramTemplate.length > 0
            ) {
              const template = this.getTemplateByName(
                outDataObj.diagramTemplate,
              );
              ...
              Object.keys(template).forEach((key) => {
                ...
                if (Array.isArray(template[key])) {
                  template[key].forEach((item) => {
                    ...
                    const newName = item[type].name.replace(
                      outDataObj.diagramTemplate,
                      outDataObj.name,
                    );
                    if (newName !== oldName) {
              ...
              this.importDiagram(template, outDataObj.forceMerge).then(
                (importedContent) => {
                  let importedDiagramIndex =
                    simApp.allDataModel.DiagramList.findIndex(
                      (diagramListItem) =>
                        diagramListItem.Diagram.name ===
                        importedContent.DiagramList[0].Diagram.name,
                    );
                  if (importedDiagramIndex !== -1) {
                    simApp.allDataModel.DiagramList[importedDiagramIndex] = {
                      Diagram: importedContent.DiagramList[0].Diagram,
                    };
                    simApp.allDataModel.DiagramList[
                      importedDiagramIndex
                    ].diagramTemplate =
                      importedContent.DiagramList[0].diagramTemplate;
                    delete simApp.allDataModel.DiagramList[importedDiagramIndex]
                      .Diagram.required;
                  }
    ...
    Sidebar.prototype.createSidebar = function (container, url) {
      var getContextMenu = function (cat, el) {
        ...
        if (cat.title == 'States') {
          cmenu = {
            ...
            preventSelect: true,
            taphold: true,
            menu: [{ title: 'Refresh', cmd: 'Refresh' }],
          ...
          cmenu = {
            ...
            taphold: true,
            menu: [{ title: 'Refresh', cmd: 'Refresh' }],
            select: function (evt, ui) {}.bind(this),
          };
        }
        //if not global only have refresh in menu for Actions and Events
        else if (
          cat.id.indexOf('Global_') == -1 &&
          (cat.title == 'Actions' || cat.title == 'Events')
        ) {
          cmenu = {
            delegate: el,
            preventContextMenuForPopup: true,
              ...
              switch (ui.cmd) {
                ...
                case 'Paste Diagram':
                  navigator.clipboard
                    .readText()
                    .then((clipped) => {
                      this.importDiagram(JSON.parse(clipped)).then(
                        (importedContent) => {
                          this.openDiagramWindow(
                            importedContent.DiagramList[0].Diagram,
                          );
                        },
                      );
                    })
      ...
      fetch(url).then((response) =>
        response.text().then((jsonStr) => {
          ...
          sbObj.sidebar.categories.forEach(
            function (cat) {
          ...
          var setUpFilters = function () {
            ...
            let diagramTypeFilterDropdownElement =
            ...
            let diagramTypes = [
              '',
              ...new Set(
                simApp.allDataModel.DiagramList.map(
                  (diagram) => diagram.Diagram.diagramLabel,
                ),
              ),
            ];
            ...
            diagramTypeFilterDropdownElement.addEventListener(
              'change',
              function () {
                ...
                let matchingDiagrams = simApp.allDataModel.DiagramList.filter(
                  (diagram) => diagram.Diagram.diagramLabel === filterType,
                );
                let /** @type {string[]} */ diagramFilterDropdownOptions = [
                    '',
                    ...matchingDiagrams.map((diagram) => diagram.Diagram.name),
                  ];
            ...
            var setupCategories = function (
              /** @type {HTMLElement | null} */ divAll,
              container,
            ) {
              if (divAll == null) return;
              sbObj.sidebar.dynamCategories.forEach(
                function (/** @type {{ title: string; id: string; }} */ cat) {
                  ...
                  sidebar.loadFilteredContent(
                    cdiv,
                    cat,
                    diagramTypeFilterDropdownElement.value,
                    diagramFilterDropdownElement.value,
                  );
                }.bind(this),
              );
            ...
            diagramFilterDropdownElement.addEventListener(
              'change',
              filterChanged,
            );
          ...
          buttonLocal.onclick = function () {
            sidebar.showDynamicSidebar('local');
          };
          buttonAll.onclick = function () {
            sidebar.showDynamicSidebar('all');
          };
          buttonGlobal.onclick = function () {
            sidebar.showDynamicSidebar('global');
          };
          ...
          $('#ContentPanel').css({
            left:
              sideBar.clientWidth +
              $('.ui-resizable-handle.ui-resizable-e').width() +
              parseInt(sideBarContainer.style.marginLeft) +
              parseInt(sideBarContainer.style.marginRight) +
              24,
          });
        }),
      );
     ...
     */
    Sidebar.prototype.getDiagramList = function (
      states,
      model = simApp.allDataModel,
    ) {
      ...
      if (model.DiagramList) {
        model.DiagramList.forEach((diagram) => {
          ...
          this.logicNodesReferencing(
            ...
            this.eventsReferencing(model, logicNode.name, 'LogicNode').forEach(
              (event) => {
                states = states.concat(
                  this.statesReferencing(model, event.name, 'Event'),
                );
      ...
      ws.get('GetSimulation?id=' + this.simId).then(
        function onSuccess(data) {
          ...
          if (resultObj.error == 0) {
            ...
            console.log(
              'Error code: ' + resultObj.error + ' - ' + resultObj.errorStr,
            );
          }
        }.bind(this),
    ...
    Sidebar.prototype.getActionByName = function (model, aName) {
      var aList;
      if (model == null) aList = simApp.allDataModel.ActionList;
      else aList = model.ActionList;
    ...
    Sidebar.prototype.getEventByName = function (model, aName) {
      var aList;
      if (model == null) aList = simApp.allDataModel.EventList;
      else aList = model.EventList;
    ...
    Sidebar.prototype.getVariableByName = function (model, aName) {
      var aList;
      if (model == null) aList = simApp.allDataModel.VariableList;
      else aList = model.VariableList;

      var varObj = null;

      for (var i = 0; i < aList.length; i++) {
        if (
          aList[i].Variable.name &&
          aList[i].Variable.name.toUpperCase() == aName.toUpperCase()
        ) {
          varObj = aList[i];
          break;
        }
    ...
    Sidebar.prototype.getExtSimByName = function (model, aName) {
      var aList;
      if (model == null) aList = simApp.allDataModel.ExtSimList;
      else aList = model.ExtSimList;
      ...
      for (var i = 0; i < aList.length; i++) {
        if (
          aList[i].ExtSim.name &&
          aList[i].ExtSim.name.toUpperCase() == aName.toUpperCase()
        ) {
          simObj = aList[i];
          break;
        }
    ...
    Sidebar.prototype.getLogicNodeByName = function (model, aName) {
      var aList;
      if (model == null) aList = simApp.allDataModel.LogicNodeList;
      else aList = model.LogicNodeList;

      var simObj = null;

      for (var i = 0; i < aList.length; i++) {
        if (
          aList[i].LogicNode.name &&
          aList[i].LogicNode.name.toUpperCase() == aName.toUpperCase()
        ) {
          simObj = aList[i];
          break;
        }
    ...
    Sidebar.prototype.getDiagramByName = function (model, aName) {
      ...
      if (model.DiagramList) {
        for (var i = 0; i < model.DiagramList.length; i++) {
          if (
            model.DiagramList[i].Diagram.name.toUpperCase() ==
            aName.toUpperCase()
          ) {
      ...
      if (this.DiagramList) {
        for (var i = 0; i < this.DiagramList.length; i++) {
          if (this.DiagramList[i].Diagram.states) {
            for (
              var j = 0;
              j < this.DiagramList[i].Diagram.states.length;
              j++
            ) {
              if (
                this.DiagramList[i].Diagram.states[j].toUpperCase() ==
                sName.toUpperCase()
              ) {
    ...
    Sidebar.prototype.getStateByName = function (model, aName) {
      ...
      for (var i = 0; i < model.StateList.length; i++) {
        if (
          model.StateList[i].State.name.toUpperCase() == aName.toUpperCase()
        ) {
     ...
     */
    Sidebar.prototype.getActionList = function (
      states,
      model = simApp.allDataModel,
    ) {
        ...
        states.forEach(
          function (state) {
            if (state.immediateActions && state.immediateActions.length > 0) {
              state.immediateActions.forEach(
                function (actionName) {
                  var actionObj = this.getActionByName(
                    simApp.allDataModel,
                    actionName,
                  );
            ...
            if (state.eventActions && state.eventActions.length > 0) {
              state.eventActions.forEach(
                function (eaObj) {
                  if (eaObj.actions && eaObj.actions.length > 0) {
                    eaObj.actions.forEach(
                      function (actionName) {
                        var actionObj = this.getActionByName(
                          simApp.allDataModel,
                          actionName,
     ...
     */
    Sidebar.prototype.getEventList = function (
      states,
      model = simApp.allDataModel,
    ) {
          ...
          function (state) {
            if (state.events && state.events.length > 0) {
              state.events.forEach(
                function (eventName) {
                  var eventObj = this.getEventByName(
                    simApp.allDataModel,
                    eventName,
                  );
          ...
          if (st.State.name == stateName) {
            ...
            if (st.immediateActions.length > 0) {
              for (var j = 0; j < st.immediateActions.length; j++) {
                ...
                if (
                  action &&
                  !diagram.ActionList.find(function (act) {
            ...
            if (st.events.length > 0) {
              for (var k = 0; k < st.events.length; k++) {
                ...
                if (
                  event &&
                  !diagram.EventList.find(function (evt) {
          ...
          if (d.Diagram.name == inData.name) {
            ...
            if (d.states) {
              for (var i = 0; i < d.states.length; i++) {
                ...
                if (
                  aState &&
                  !diagram.StateList.find(function (st) {
                    return st.State.name == aState.State.name;
                  })
                )
    ...
    Sidebar.prototype.replaceNames = function (
      oldName,
      newName,
      type,
      model,
      ...
      if (model == null) model = simApp.allDataModel;
      //try and replace for all object referencing calls
      this.statesReferencing(
        model,
        oldName,
        type,
        false,
      ...
      if (updateSidebar) {
        this.alterSideBarListsItem(
          oldName,
          newName,
          new Set(['All', 'Global', 'Local']),
          type,
        );
      }
    };
    ...
    Sidebar.prototype.addNewAction = function (newAction, parent, isImmediate) {
      ...
      if (mainModel.ActionList) {
        ...
        if (!act) {
          ...
          var globalContainer = document.getElementById(
            'Global_ActionsPanel_id',
          );
          var container = document.getElementById('ActionsPanel_id');
          if (container) {
            newAction.ui_el = this.addSectionItem(
              container,
              'Actions',
    ...
    //parent - either the ImmediateAction array or the EventAction's actions array.
    //isImmediate - true if this is an immediate action add, false EventAction's action.
    Sidebar.prototype.addNewLocalAction = function (
      newAction,
      parent,
      isImmediate,
    ) {
    ...
    //---------------------------------------------------
    Sidebar.prototype.addNewLocalEvent = function (
      newEvent,
      parent,
      existingAsNew,
    ) {
      ...
      if (mainModel.EventList) {
        ...
        if (!evt) {
          var idx = mainModel.EventList.push(newEvent);
          newEvent.Event.id = idx;
          if (!existingAsNew && parent) {
            //parent is the state's events array if set. If not set then it exists in parent and is just being saved as a new item.
            parent.push(newEvent.Event.name);
          }
    ...
    Sidebar.prototype.addNewVariable = function (newVariable, parent) {
      ...
      if (mainModel.VariableList) {
        ...
        if (!varr) {
          ...
          if (container) {
            newVariable.ui_el = this.addSectionItem(
              container,
              'Variables',
    ...
    Sidebar.prototype.addNewLogicTree = function (newLogicNode, parent) {
      ...
      if (mainModel.LogicNodeList) {
        var existing = this.getLogicNodeByName(
          mainModel,
          newLogicNode.LogicNode.name,
        );
        if (!existing) {
          ...
          if (container) {
            newLogicNode.ui_el = this.addSectionItem(
              container,
              'Logic Tree',
              newLogicNode.LogicNode.name,
              newLogicNode.LogicNode,
            );
            sortDOMList(container);
          }
          return true;
    ...
    Sidebar.prototype.addNewExtSim = function (newExtSim, parent) {
      ...
      if (!sim) {
        ...
        if (container) {
          newExtSim.ui_el = this.addSectionItem(
            container,
            'External Sims',
            newExtSim.ExtSim.name,
            newExtSim.ExtSim,
          );
          sortDOMList(container);
        }
    ...
    Sidebar.prototype.addNewState = function (newState) {
      ...
      if (mainModel.StateList) {
        ...
        if (container) {
          newState.ui_el = this.addSectionItem(
            container,
            'States',
            newState.State.name,
            newState.State,
    ...
    Sidebar.prototype.cleanDataModel = function (diagram) {
      ...
      if (diagram.StateList)
        for (var i = 0; i < diagram.StateList.length; i++) {
          ...
          if (typeof diagram.StateList[i]['actionCell'] !== 'undefined')
            delete diagram.StateList[i]['actionCell'];
          if (typeof diagram.StateList[i]['dataType'] !== 'undefined')
            delete diagram.StateList[i]['dataType'];
          if (typeof diagram.StateList[i]['eventCell'] !== 'undefined')
            delete diagram.StateList[i]['eventCell'];
          if (typeof diagram.StateList[i]['iActions'] !== 'undefined')
            delete diagram.StateList[i]['iActions'];
          if (typeof diagram.StateList[i]['iEvents'] !== 'undefined')
            delete diagram.StateList[i]['iEvents'];
          if (typeof diagram.StateList[i]['ownerCell'] !== 'undefined')
            delete diagram.StateList[i]['ownerCell'];
          if (typeof diagram.StateList[i]['ownerID'] !== 'undefined')
            delete diagram.StateList[i]['ownerID'];
        }
    ...
    //-------------------------------------------
    Sidebar.prototype.editDiagramType = function (
      diagramName,
      oldType,
      newType,
    ) {
      ...
      var wnd = mxWindow.createFrameWindow(
        ...
        function (btn, retObj) {
          if (btn in SetOf(['OK', 'Save As New'])) {
            ...
            simApp.modelChanged = true;
            if (el && !asNew) {
              // el is not null when editing an existing action.
    ...
    Sidebar.prototype.editEventProperties = function (dataObj, el) {
      ...
      for (var i = 0; i < nodeList.length; i++) {
        ...
        if (ln.isRoot) logicNodes.push(ln);
      }
    ...
    Sidebar.prototype.alterSideBarListsItem = function (
      itemName,
      newName,
      tabs,
      type,
      ...
      if (type == 'LogicNode') {
        ...
        if (index != null) {
          ...
          if (newName == null) element.parentNode.removeChild(element);
          else element.innerHTML = newName;
        }
        return;
      }
        ...
        if (idx >= 0) {
          ...
          for (var i = 0; i < dataObj.states.length; i++) {
            ...
            if (astate) {
              ...
              if (astate.ui_el) astate.ui_el.remove();
            }
          }
        }
      }
      ...
      if (mainModel.StateList) {
        ...
        if (idx >= 0) {
          ...
          for (var i = 0; i < dataObj.immediateActions.length; i++) {
            ...
            if (action.ui_el) action.ui_el.remove();
          }
    ...
    //------------------------------------------
    Sidebar.prototype.deleteAction = function (
      dataObj,
      fullDelete,
      state,
      event,
      ...
      var refCnt = this.statesReferencing(
        mainModel,
        dataObj.name,
        'Action',
        fullDelete,
    ...
    Sidebar.prototype.eventsReferencing = function (
      model,
      name,
      type,
      del = false,
      ...
      var refs = [];
      if (model == null)
        //use the entire model not just what was given
        model = simApp.allDataModel;
              ...
              if (cur.varNames) {
                ...
                if (j > -1) {
                  if (del) {
                    ...
                    if (replaceName != null) cur.varNames[j] = replaceName;
                    refs.push(cur);
                  }
                }
              }
              ...
              if (cur.triggerStates) {
                ...
                if (j > -1) {
                  if (del) {
                    ...
                    if (replaceName != null) cur.triggerStates[j] = replaceName;
                    refs.push(cur);
                  }
                }
              }
            ...
            case 'LogicNode':
              ...
              if (cur.logicTop && cur.logicTop == name) {
                if (del) {
                  ...
                  if (replaceName != null) cur.logicTop = replaceName;
                  refs.push(cur);
                }
              }
              break;
    ...
    Sidebar.prototype.actionsReferencing = function (
      model,
      name,
      type,
      del = false,
            ...
            case 'ExtSim':
              ...
              if (cur.extSim && cur.extSim == name) {
                if (del) {
                  ...
                  if (replaceName != null) cur.extSim = replaceName;
                  refs.push(cur);
                }
              }
              break;
            case 'Variable':
              ...
              if (cur.newStates) {
                cur.newStates.forEach((newState, i) => {
                  if (
                    typeof newState.varProb === 'string' &&
                    newState.varProb === name
                  ) {
                    refs.push(cur);
                    if (replaceName) {
              ...
              if (cur.variableName && cur.variableName == name) {
                if (del) {
                  ...
                  if (replaceName != null) cur.variableName = replaceName;
                  refs.push(cur);
                }
              }
              if (cur.scriptCode && replaceName) {
                cur.scriptCode = this.replaceNamesInText(
                  cur.scriptCode,
                  name,
                  replaceName,
                );
              ...
              if (cur.sim3DVariable && cur.sim3DVariable == name) {
                if (del) {
                  ...
                  if (replaceName != null) cur.sim3DVariable = replaceName;
                  refs.push(cur);
                }
              }
              //atCngVarVal
              if (cur.codeVariables) {
                ...
                if (j > -1) {
                  if (del) {
                    ...
                    if (replaceName != null) cur.codeVariables[j] = replaceName;
                    refs.push(cur);
                  }
                }
              }
              ...
              if (cur.makeInputFileCode && replaceName) {
                cur.makeInputFileCode = this.replaceNamesInText(
                  cur.makeInputFileCode,
                  name,
                  replaceName,
                );
              }
              if (cur.processOutputFileCode && replaceName) {
                cur.processOutputFileCode = this.replaceNamesInText(
                  cur.processOutputFileCode,
                  name,
                  replaceName,
                );
              }
    ...
    Sidebar.prototype.statesReferencing = function (
      model,
      name,
      type,
      delAll = false,
              ...
              if (cur.events) {
                for (var j = 0; j < cur.events.length; j++) {
                  if (cur.events[j] == name) {
                    if (
                      delAll ||
                      (delForState && delForState.name == cur.name)
                    ) {
                      cur.events.splice(j, 1);
                      cur.eventActions.splice(j, 1);
                    } else {
                      if (replaceName != null) cur.events[j] = replaceName;
                      refs.push(cur);
                    }
                  }
                }
                  ...
                  for (var l = 0; l < cur.eventActions[k].actions.length; ++l) {
                    if (cur.eventActions[k].actions[l] == name) {
                      if (
                        delAll ||
                        (delForState &&
                          delForState.name == cur.name &&
                          (!delForEv || delForEv == cur.events[k]))
                      ) {
    ...
    Sidebar.prototype.diagramsReferencing = function (
      model,
      name,
      type,
      del = false,
              ...
              if (cur.states) {
                ...
                if (j > -1) {
                  if (del) {
                    ...
                    if (replaceName != null) cur.states[j] = replaceName;
                    refs.push(cur);
                  }
                }
              }
    ...
    Sidebar.prototype.logicNodesReferencing = function (
      model,
      name,
      type,
      del = false,
              ...
              if (cur.compChildren) {
                ...
                if (j > -1) {
                  ...
                  if (del) {
                    ...
                    if (replaceName != null) cur.compChildren[j] = replaceName;
                  }
                }
              }
              break;
    ...
    Sidebar.prototype.variableReferencing = function (
      model,
      name,
      type,
      del = false,
    ...
    Sidebar.prototype.extSimsReferencing = function (
      model,
      name,
      type,
      del = false,
    ...
    Sidebar.prototype.numberOfInstancesActions = function (name, states) {
      ...
      actions.forEach(
        function (item) {
          if (item.Action.name == name) count += 1;
        }.bind(this),
      );
      return count;
    };
    ...
    Sidebar.prototype.numberOfInstancesEvents = function (name, states) {
      ...
      if (states) {
        ...
        events.forEach(
          function (item) {
            if (item.Event.name == name) count += 1;
          }.bind(this),
        );
        return count;
      }
    ...
    Sidebar.prototype.addLocalTemplate = function (templateObj) {
      ...
      localTemplates = localTemplates.filter(
    ...
    Sidebar.prototype.getTemplateByName = function (templateName) {
      return this.getLocalTemplates().find(
    ...
    Sidebar.prototype.addDiagramToSection = function (ol, item) {
      ...
      if (!item.Diagram.diagramLabel) {
        item.Diagram.diagramLabel = item.Diagram.diagramType.substring(
          2,
          item.Diagram.diagramType.length,
        );
      }
      ...
      var l = Array.from(ls).filter(
        (el) => el.innerText == item.Diagram.diagramLabel,
      )[0]; //search for parent node.
          ...
          switch (ui.cmd) {
            case 'Edit':
              if (ui.target.context.dataObject)
                this.editDiagramProperties(
                  ui.target.context.dataObject,
                  ui.target.context,
                );
              break;
            case 'Open':
            ...
            case 'Delete':
              if (ui.target.context.dataObject) {
                MessageBox.confirm(
                  'Deleting a Diagram',
                  'Are you sure you want to delete the highlighted diagram?',
                  ['Yes', 'Cancel'],
                  function (btn, evt) {
                    if (btn === 'Yes') {
                      this.closeDeletedWindow(
                        ui.target.context.dataObject.name,
                      );
                      this.deleteDiagram(
                        ui.target.context.dataObject,
                        ui.target.context,
            ...
            case 'Export':
              if (ui.target.context.dataObject) {
                ...
                const dataObject = this.exportDiagram(
                  ui.target.context.dataObject,
                );
                a.setAttribute(
                  'href',
    ...
    Sidebar.prototype.exportDiagram = function (dataObject, model = undefined) {
      const StateList = this.statesReferencing(
        model,
        dataObject.name,
        'Diagram',
      );
      ...
      exportObject.DiagramList = simApp.mainApp.cloneDataModel(
        exportObject.DiagramList,
      );
      exportObject.ExtSimList = simApp.mainApp.cloneDataModel(
        exportObject.ExtSimList,
      );
      exportObject.StateList = simApp.mainApp.cloneDataModel(
        exportObject.StateList,
      );
      exportObject.ActionList = simApp.mainApp.cloneDataModel(
        exportObject.ActionList,
      );
      exportObject.EventList = simApp.mainApp.cloneDataModel(
        exportObject.EventList,
      );
      exportObject.LogicNodeList = simApp.mainApp.cloneDataModel(
        exportObject.LogicNodeList,
      );
      exportObject.VariableList = simApp.mainApp.cloneDataModel(
        exportObject.VariableList,
      );
      return exportObject;
    };
    ...
    Sidebar.prototype.LogicNodeExists = function (logicNodeName) {
      return (
        this.getLogicNodeByName(simApp.allDataModel, logicNodeName) !== null
      );
    };
     ...
     */
    Sidebar.prototype.importDiagram = function (
      importedContent,
      forceMerge = false,
    ) {
            ...
            if (btn === 'OK') {
              ...
              outDataObj.entries.forEach((entry) => {
                if (entry.data.excluded) {
                  ...
                  let excludedIndex = outDataObj.model[
                    `${entry.type}List`
                  ].findIndex(
                    (item) => item[entry.type].name === entry.data.name,
                  );
                  outDataObj.model[`${entry.type}List`].splice(
                    excludedIndex,
                    1,
                  );
                } else {
    ...
    Sidebar.prototype.addDiagramSectionItem = function (
      container,
      diagramList,
    ) {
          ...
          $(li).contextmenu({
            ...
            select: function (evt, ui) {
              ...
              switch (ui.cmd) {
                ...
                case 'Delete':
                  if (ui.target.context.dataObject) {
                    var actionList = getActionsUsingVariable(
                      this,
                      ui.target.context.dataObject.name,
                    );
                    if (actionList.length < 1) {
                      MessageBox.confirm(
                        'Deleting a variable',
                        'Are you sure you want to delete the highlighted Variable?',
                        ['Yes', 'Cancel'],
                        function (btn, evt) {
                          if (btn == 'Yes') {
                            this.closeDeletedWindow(
                      ...
                      var message =
                        'The variable ' +
                        ui.target.context.dataObject.name +
                        ' is used in the following actions: ';
                      for (var i = 0; i < actionList.length; i++) {
            ...
            select: function (evt, ui) {
              ...
              switch (ui.cmd) {
                ...
                case 'Delete':
                  if (ui.target.context.dataObject) {
                    MessageBox.confirm(
                      'Deleting a Logic Node',
                      'Are you sure you want to delete the highlighted Logic Node?',
                      ['Yes', 'Cancel'],
                      function (btn, evt) {
                        if (btn === 'Yes') {
                          this.closeDeletedWindow(
              ...
              switch (ui.cmd) {
                ...
                case 'Delete':
                  if (ui.target.context.dataObject) {
                    MessageBox.confirm(
                      'Deleting a Action',
                      'Are you sure you want to delete the highlighted Action?',
                      ['Yes', 'Cancel'],
                      function (btn, evt) {
                        if (btn == 'Yes') {
                          this.closeDeletedWindow(
              ...
              switch (ui.cmd) {
                ...
                case 'Delete':
                  if (ui.target.context.dataObject) {
                    MessageBox.confirm(
                      'Deleting a Event',
                      'Are you sure you want to delete the highlighted Event?',
                      ['Yes', 'Cancel'],
                      function (btn, evt) {
                        if (btn == 'Yes') {
                          this.closeDeletedWindow(
              ...
              switch (ui.cmd) {
                ...
                case 'Delete':
                  if (ui.target.context.dataObject) {
                    MessageBox.confirm(
                      'Deleting a State',
                      'Are you sure you want to delete the highlighted state?',
                      ['Yes', 'Cancel'],
                      function (btn, evt) {
                        if (btn == 'Yes') {
                          this.closeDeletedWindow(
            ...
            select: function (evt, ui) {
              ...
              switch (ui.cmd) {
                ...
                case 'Delete':
                  if (ui.target.context.dataObject) {
                    MessageBox.confirm(
                      'Deleting a External Sim',
                      'Are you sure you want to delete the highlighted External Sim?',
                      ['Yes', 'Cancel'],
                      function (btn, evt) {
                        if (btn == 'Yes')
                          this.closeDeletedWindow(
            ...
            if (id == 'Global_ActionsPanel_id') {
              this.ActionList.forEach(
                function (item) {
                  ...
                  if (mainItem) {
                    item.ui_el = this.addSectionItem(
                      container,
                      section,
                      item.Action.name,
                      item.Action,
    ...
    Sidebar.prototype.getFilteredLookup = function (
      container,
      section,
      id,
      /** @type {string}} */ diagramType,
      /** @type {string}} */ diagramName,
    ) {
      switch (section) {
        case 'Diagrams':
          if (this.DiagramList) {
        ...
        case 'Actions':
          if (this.ActionList) {
            if (id == 'Global_ActionsPanel_id') {
              ...
              // It should always come here since filters are only in the "All" tab.
              let states = this.statesReferencing(
                null,
                diagramName,
                'Diagram',
              ).forEach((state) => {
                let actions = this.actionsReferencing(
                  null,
                  state.name,
                  'State',
                ).forEach(
        ...
        case 'Events':
          if (this.EventList) {
            if (id == 'Global_EventsPanel_id') {
              ...
              this.statesReferencing(null, diagramName, 'Diagram').forEach(
                (state) => {
      ...
      if (eventContainer) {
        ...
        for (var i = 0; i < events.length; i++) {
          if (!events[i].Event.mainItem) {
            events[i].ui_el = this.addSectionItem(
              eventContainer,
              'Events',
              events[i].Event.name,
              events[i].Event,
      ...
      if (actionContainer) {
        ...
        for (var i = 0; i < actions.length; i++) {
          if (!actions[i].Action.mainItem) {
            actions[i].ui_el = this.addSectionItem(
              actionContainer,
              'Actions',
    ...
    Sidebar.prototype.emptyLocalPanels = function () {
      ...
      var variableContainer = document.getElementById(
        'Local_VariablesPanel_id',
      );
      if (variableContainer) {
        variableContainer.innerHTML = '';
      }
    ...
    Sidebar.prototype.loadFilteredContent = function (
      container,
      catInfo,
      /** @type {string}} */ diagramType,
      /** @type {string}} */ diagramName,
      ...
      if (diagramName === '') {
        this.loadContent(container, catInfo);
      } else {
        var lookup = this.getFilteredLookup(
          container,
          catInfo.title,
          catInfo.id,
          diagramType,
    ...
    Sidebar.prototype.windowIsClosed = function (windowName) {
      return (this.openWindows = this.openWindows.filter(
        (window) => window.id !== windowName,
      ));
    };
    ...
    Sidebar.prototype.closeDeletedWindow = function (windowToDelete) {
      var matchingWindow = this.openWindows.find(
        (window) => window.id === windowToDelete,
      );
    ...
    if (!isChrome()) {
      alert(
        'EMRALD Model Editor is currently only supported by Google Chrome. Please switch browsers for the best experience using EMRALD.',
      );
    }
